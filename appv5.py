...
def obtener_archivos_xlsm():
    import requests
    try:
        url_api = "https://api.github.com/repos/mpintoponce/Grupo_5_Python/contents"
        response = requests.get(url_api)
        response.raise_for_status()
        archivos = response.json()
        return [
            f["name"] for f in archivos
            if f["name"].endswith(".xlsm") and "CUMPLIMIENTO" in f["name"].upper()
        ]
    except Exception as e:
        st.error(f"Error obteniendo archivos desde GitHub: {e}")
        return []
# --- CARGA DE DATOS ---
# @st.cache_data
# def load_data():
...
def load_data(nombre_archivo:str):
    import requests
    import io

    url = f"https://github.com/mpintoponce/Grupo_5_Python/raw/main/{nombre_archivo}"

    try:
        response = requests.get(url)
        response.raise_for_status()
        archivo_excel = io.BytesIO(response.content)

        df_plan = pd.read_excel(archivo_excel, sheet_name='IW38 PLAN SEMANAL', engine='openpyxl')
        df_semana = pd.read_excel(archivo_excel, sheet_name='IW38 SEMANA CAL', engine='openpyxl')
        df_bl = pd.read_excel(archivo_excel, sheet_name='BL MENSUALOT', engine='openpyxl')
        df_cm = pd.read_excel(archivo_excel, sheet_name='CMENSUAL', engine='openpyxl')
        df_tendencia = pd.read_excel(archivo_excel, sheet_name='RESUMEN ANUAL', engine='openpyxl')
...
# === Selector de archivo .xlsm desde GitHub ===
archivos_disponibles = obtener_archivos_xlsm()
mapa_semanas = {}
for archivo in archivos_disponibles:
    import re
    match = re.search(r'SEM(\d{1,3})', archivo.upper())  # Solo 1 a 3 dígitos después de SEM
    if match:
        semana = int(match.group(1))  # Asegura que es un número válido
        label = f"Semana {semana}"
    else:
        label = archivo  # fallback
    mapa_semanas[label] = archivo

# Mostrar selectbox con etiquetas personalizadas
label_seleccionado = st.sidebar.selectbox("Seleccionar Semana", list(mapa_semanas.keys()))
archivo_seleccionado = mapa_semanas[label_seleccionado]

if not archivo_seleccionado:
    st.warning("No se ha seleccionado ningún archivo válido.")
    st.stop()

df_plan, df_semana, df_bl, df_cm, df_tendencia = load_data(archivo_seleccionado)
# --- PARÁMETROS DE FECHA ---
fecha_actual = datetime.today()
numero_semana = fecha_actual.isocalendar().week
anio_dos_digitos = fecha_actual.strftime('%y')
